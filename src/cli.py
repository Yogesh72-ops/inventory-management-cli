...
def prompt_int(msg: str, min_value: int = None) -> int:
    while True:
        try:
            value = int(input(msg))
        except ValueError:
...
def prompt_float(msg: str, min_value: float = None) -> float:
    while True:
        try:
...
def handle_choice(choice: str) -> bool:
    ...
    if choice == "1":
        name = prompt_nonempty("Supplier name: ")
        ...
        price = prompt_float("Price: ", min_value=0)
        stock = prompt_int("Initial stock: ", min_value=0)
        reorder = prompt_int("Reorder level (e.g. 10): ", min_value=0)
        ...
        qty = prompt_int("New stock quantity: ", min_value=0)
        ...
        name = prompt_nonempty("Customer name: ")
        ...
        while True:
            ...
            if not pid_input.strip():
                break
            pid = int(pid_input)
            qty = prompt_int("  Quantity: ", min_value=1)
